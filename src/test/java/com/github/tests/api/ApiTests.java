...
public class ApiTests extends TestBase {
    ...
    @Test
    @Description("Checks that user data matches to entered")
    ...
    void userDataMatchingTest() {
        ...
        step("Check that user data matches to before entered", () -> {
            ...
            assertThat(userData.getLogin()).isEqualTo(config.getLogin());
            assertThat(userData.getEmail()).isEqualTo(config.getEmail());
    ...
    void createNewRepoTest() {
        ...
        step("Check that current repository info matches to before entered ", () -> {
            ...
            assertThat(repoInfo.fullName()).isEqualTo(
                    String.format("%s/%s", config.getLogin(), repoInfo.name()));
            assertThat(repoInfo.privateType()).isEqualTo(repositoryModel.privateType());
            assertThat(repoInfo.htmlUrl()).isEqualTo(
                    String.format("%s/%s/%s", baseConfig.getBaseUrl(), config.getLogin(), repoInfo.name()));
            assertThat(repoInfo.description()).isEqualTo(repositoryModel.description());
            assertThat(repoInfo.owner().login()).isEqualTo(config.getLogin());
    ...
    @Description("Checks that adding invalid email is impossible")
